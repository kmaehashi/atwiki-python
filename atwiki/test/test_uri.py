...
class AtWikiURITest(TestCase):
  ...
  def test_path_output(self):
    ...
    self.assertEqual(uri.tag(), '{0}/tag/?p=0&sort=tag'.format(base))
    self.assertEqual(uri.tag('test'), '{0}/tag/test?p=0&sort=tag'.format(base))
    self.assertEqual(uri.tag('test', index=1), '{0}/tag/test?p=1&sort=tag'.format(base))
